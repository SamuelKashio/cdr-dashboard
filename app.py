...
# Escenarios de llamada con etiqueta y color
ESCENARIOS = {
...
def norm_num(n):
    """Normaliza número de teléfono a últimos 9 dígitos para comparación."""
...
def fetch_cdrs(date_start=None, date_end=None, recent=False, live=False, progress_cb=None):
    if live:
        try:
            r = requests.get("https://callmyway.com/getCdrs.php",
    ...
    if recent:
        try:
            r = requests.get("https://callmyway.com/getCdrs.php",
                params={"username":_U,"password":_P,"recent":1,"format":"json"}, timeout=20)
            r.raise_for_status()
            data = r.json()
            cdrs = data.get("cdrs", data) if isinstance(data, dict) else data
            return pd.DataFrame(cdrs or []), None
...
def procesar(df_raw):
    ...
    # ── Salientes: agente → cliente externo ───────────────────────────────────
    mask_sal = (
        (df["type"] == "outgoing") &
    ...
    # ── Entrantes: clasificación por escenario ────────────────────────────────
...
# ── Cumplimiento de callbacks (seguimiento de llamadas perdidas) ───────────────
VENTANA_CB = pd.Timedelta(minutes=5)

def calcular_cumplimiento(df_ent, df_sal):
    """
    Para cada llamada perdida verifica si en ≤5 min:
      A) El agente hizo una llamada saliente conectada al mismo número, o
      B) El cliente volvió a llamar y fue atendido.
    ...
    perdidas = df_ent[
        (df_ent["atendida"] == False) &
        (df_ent["escenario"].isin(esc_responsable))
    ].copy().sort_values("detect_time")

    if perdidas.empty: return pd.DataFrame()

    # Pre-normalizar números para performance
    if not df_sal.empty and "numero_cliente" in df_sal.columns:
        df_sal = df_sal.copy()
        df_sal["_num"] = df_sal["numero_cliente"].apply(norm_num)
    if not df_ent.empty:
        df_ent = df_ent.copy()
        df_ent["_num"] = df_ent["numero_cliente"].apply(norm_num)
    ...
    for _, row in perdidas.iterrows():
        ...
        # A) Agente llamó de vuelta (saliente conectada dentro de ventana)
        cb_sal = pd.DataFrame()
        if not df_sal.empty and "detect_time" in df_sal.columns:
            cb_sal = df_sal[
                (df_sal["_num"] == num) &
                (df_sal["detect_time"] > t0) &
                (df_sal["detect_time"] <= t_lim) &
                (df_sal["atendida"] == True)
            ]

        # B) Cliente volvió a llamar y fue atendido dentro de ventana
        cb_ent = pd.DataFrame()
        if not df_ent.empty and "detect_time" in df_ent.columns:
            cb_ent = df_ent[
                (df_ent["_num"] == num) &
                (df_ent["detect_time"] > t0) &
                (df_ent["detect_time"] <= t_lim) &
                (df_ent["atendida"] == True)
            ]

        if not cb_sal.empty:
            tipo   = "📞 Agente llamó"
            t_cb   = cb_sal["detect_time"].min()
            ag_cb  = cb_sal.iloc[0].get("agente","—")
            seg    = int((t_cb - t0).total_seconds())
        elif not cb_ent.empty:
            tipo   = "↩️ Cliente volvió"
            t_cb   = cb_ent["detect_time"].min()
            ag_cb  = cb_ent.iloc[0].get("agente","—")
            seg    = int((t_cb - t0).total_seconds())
        else:
            tipo   = "❌ Sin resolución"
            t_cb   = None
        ...
        resultados.append({
            "Fecha/Hora":        t0,
            "Número":            row["numero_cliente"],
            "Responsable":       row.get("responsable","—"),
            "Escenario":         esc_es(row.get("escenario","")),
            "Resolución":        tipo,
            "Tiempo respuesta":  fmt_dur(seg) if seg is not None else "> 5 min",
            "Agente resolvió":   ag_cb,
            "Cumplimiento":      tipo != "❌ Sin resolución",
            "_seg":              seg,
        })
...
with st.sidebar:
    ...
    st.markdown(f"<div style='background:#0C0F1C;border:1px solid rgba(80,120,200,.15);border-radius:8px;padding:10px 12px;margin-bottom:16px'>"
                f"<div style='color:#1A3050;font-size:10px;font-family:JetBrains Mono,monospace;letter-spacing:.5px'>CUENTA</div>"
...
if live_mode:
    ...
    for i,(ag_id,ag_nombre) in enumerate(AGENTES_SIN_CENTRAL.items()):
        ...
        with cols_ag[i%3]:
            st.markdown(f"""<div style='background:#0C0F1C;border:1px solid {borde};border-top:3px solid {dot};border-radius:10px;padding:16px;margin-bottom:14px'>
    ...
    if sin_asignar:
        ...
        for c in sin_asignar:
            ...
            st.markdown(f"""<div style='background:#0C0F1C;border:1px solid rgba(234,179,8,.25);border-left:3px solid #EAB308;border-radius:10px;padding:14px 18px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center'>
        ...
        st.markdown("""<div style='text-align:center;padding:40px;background:#0C0F1C;border:1px solid rgba(255,255,255,0.04);border-radius:12px;margin-top:8px'>
...
if df_ent.empty and df_sal.empty:
...
<div style='display:flex;align-items:flex-end;justify-content:space-between;
  ...
  <div>
    <div style='font-size:22px;font-weight:300;color:#C8D8E8'>Panel de Supervisor · Soporte</div>
    <div style='font-size:11px;color:#1A3050;font-family:JetBrains Mono,monospace;margin-top:4px'>
        {lbl} · {n_ent} entrantes · {n_sal} salientes</div>
  </div>
...
with tab_ov:
    ...
    with r3:
        if not df_ent.empty and "escenario" in df_ent.columns:
            esc_counts = df_ent["escenario"].value_counts().reset_index()
            esc_counts.columns = ["esc","n"]
            esc_counts["label"] = esc_counts["esc"].apply(esc_es)
            esc_counts["color"] = esc_counts["esc"].apply(esc_color)
            esc_counts = esc_counts.sort_values("n",ascending=True)
            fig_esc = go.Figure(go.Bar(x=esc_counts["n"],y=esc_counts["label"],orientation="h",
                marker_color=esc_counts["color"],marker_line_width=0,
                text=esc_counts["n"],textposition="outside",textfont=dict(size=11,color="#2A4060")))
            fig_esc.update_layout(height=240,**P,
...
with tab_ent:
    if df_ent.empty: st.info("Sin llamadas entrantes.")
        ...
        ds = ds.rename(columns={"detect_time":"Fecha/Hora","numero_cliente":"Número",
            "escenario_es":"Escenario","agente":"Contestó","responsable":"Responsable",
            "agente_turno":"Turno","duracion":"Duración","espera_total":"Espera","n_intentos":"Intentos"})
        st.caption(f"{len(dv):,} llamadas mostradas")
        st.dataframe(ds,use_container_width=True,height=460,hide_index=True)
        ec1,ec2 = st.columns(2)
        with ec1: st.download_button("⬇ Exportar entrantes",data=df_ent.to_csv(index=False).encode("utf-8-sig"),file_name=f"entrantes_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv")
        with ec2: st.download_button("⬇ Exportar perdidas", data=df_ent[df_ent["atendida"]==False].to_csv(index=False).encode("utf-8-sig"),file_name=f"perdidas_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv")
...
with tab_ag:
    ...
    for aid,nombre in AGENTES_SIN_CENTRAL.items():
        ...
        durs = ea["duracion"].dropna().tolist()
        per_t = len(df_ent[(df_ent.get("responsable",pd.Series())== nombre) & (df_ent.get("atendida",pd.Series())==False)]) if not df_ent.empty and "responsable" in df_ent.columns else 0
    ...
    for i,ag in enumerate(ag_data):
        with cols3[i%3]:
            ...
            tot = ag["ent_at"]+ag["per_turno"]
            pct = round(ag["ent_at"]/tot*100) if tot else 0
    ...
    if ag_data:
        ...
        with gc2:
            ...
            fig_d2.update_traces(marker_line_width=0)
    ...
    if df_ent.empty: st.info("Sin datos para analizar turnos.")
        ...
        if "responsable" in df_ent.columns:
            ...
            for resp in sorted(df_ent["responsable"].dropna().unique()):
                ...
                tot=len(sub); at=int((sub["atendida"]==True).sum())
                drs = sub[sub["atendida"]==True]["duracion"].dropna()
...
# ── TAB 5: SEGUIMIENTO (callbacks) ────────────────────────────────────────────
with tab_seg:
    st.markdown("#### 📋 Seguimiento de llamadas perdidas — ventana de 5 minutos")
    st.markdown("""<div style='background:#0C0F1C;border:1px solid rgba(80,120,200,.15);border-radius:8px;padding:12px 16px;margin-bottom:20px;font-size:13px;color:#4A7ABA'>
    Se registra <b>cumplimiento</b> si: el agente llamó de vuelta al cliente (<i>saliente conectada</i>), <b>o</b> el cliente volvió a llamar y fue atendido,
    todo dentro de los <b>5 minutos</b> siguientes a la llamada perdida.
    Solo aplica a escenarios con responsabilidad del agente (no aplica a llamadas que colgaron en IVR).
    </div>""", unsafe_allow_html=True)
    ...
    if df_cb.empty:
        ...
        total_per  = len(df_cb)
        cumpl      = int(df_cb["Cumplimiento"].sum())
        no_cumpl   = total_per - cumpl
        pct_cumpl  = round(cumpl/total_per*100) if total_per else 0
        avg_t_resp = df_cb[df_cb["_seg"].notna()]["_seg"].mean()
        ...
        sc4.metric("Tiempo prom. respuesta",  fmt_dur(int(avg_t_resp)) if not pd.isna(avg_t_resp) else "—")
        st.markdown("<br>",unsafe_allow_html=True)

        # Gráfico: cumplimiento por agente
        if "Responsable" in df_cb.columns:
            ...
            fig_cb.update_traces(marker_line_width=0)
        ...
        st.download_button("⬇ Exportar seguimiento CSV",
            ...
            file_name=f"seguimiento_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv")
    ...
    if df_ent.empty: st.info("Sin datos de clientes.")
        ...
        if not cp.empty:
            ...
            for _,row in cp.iterrows():
                ...
                st.markdown(f"""<div style='background:#0C0F1C;border:1px solid rgba(239,68,68,.2);border-radius:8px;padding:10px 14px;margin-bottom:6px;display:flex;align-items:center;justify-content:space-between'>
        ...
        fig_cl.update_layout(height=360,**P,xaxis=dict(gridcolor="rgba(255,255,255,.03)",title="Llamadas"),yaxis_title="",
        ...
        cl_s = cl_s.rename(columns={"numero_cliente":"Número","total":"Total","atendidas":"Atendidas","perdidas":"Perdidas","pct_at":"% At.","ag_frec":"Agente frecuente","ultima":"Última llamada"})
